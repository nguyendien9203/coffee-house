...
//        try {
//            stm = connection.prepareStatement("SELECT * FROM users WHERE username = ? AND password = ?");
...
//    }
    ...
    public boolean registerUser(User user) {
        try {
            String sql = "INSERT INTO users (role, username, password, status) VALUES (?, ?, ?, ?);";
            stm = connection.prepareStatement(sql);
            stm.setString(1, user.getRole().toString());
            stm.setString(2, user.getUsername());
            stm.setString(3, user.getPassword());
            stm.setString(4, user.getStatus().toString());
            stm.executeUpdate();
            return true;
